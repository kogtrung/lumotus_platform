...
function GlobalLeaderboardRow({ entry, isCurrentUser }: { entry: GlobalRow, isCurrentUser?: boolean }) {
  ...
  return (
    ...
    >
      <div className={`relative flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-lg bg-[#1A1520] text-xs font-black ${getRankText(entry.rank)}`}>
        #{entry.rank}
        {getRankIcon(entry.rank)}
      </div>
      <Avatar url={entry.avatarUrl} name={entry.username} />
      <div className="min-w-0 flex-1">
        <p className={`truncate text-xs sm:text-sm font-bold flex items-center gap-2 ${isTop3 ? getRankText(entry.rank) : (isCurrentUser ? 'text-blue-400' : 'text-[#F5F0FA]')}`}>
          {entry.username}
          {isCurrentUser && <span className="rounded bg-blue-500/20 px-1.5 py-0.5 text-[8px] font-bold text-blue-400 border border-blue-500/30">BẠN</span>}
        </p>
        <div className="flex flex-wrap items-center gap-1.5 mt-0.5 text-[9px] text-[#8B7A9E]">
          <span className="flex items-center gap-1"><Clock className="h-3 w-3 text-cyan-400" /> {formatTime(entry.totalTimeSeconds)}</span>
          <span className="w-1 h-1 rounded-full bg-[#8B7A9E]/40" />
      ...
      <div className="shrink-0 text-right min-w-[50px]">
        <p className={`text-sm sm:text-base font-black ${isTop3 ? getRankText(entry.rank) : 'text-[#EC4899]'}`}>{score}</p>
        <p className="text-[9px] text-[#8B7A9E]">TB</p>
      </div>
    </motion.div>
  )
}
...
function WeeklyLeaderboardRow({ entry, isCurrentUser }: { entry: LeaderboardEntry, isCurrentUser?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <Avatar url={entry.avatarUrl} name={entry.username} />
      <div className="min-w-0 flex-1">
        <p className={`truncate text-xs sm:text-sm font-bold flex items-center gap-2 ${isTop3 ? getRankText(entry.rank) : (isCurrentUser ? 'text-pink-400' : 'text-[#F5F0FA]')}`}>
          {entry.username}
          {isCurrentUser && <span className="rounded bg-pink-500/20 px-1.5 py-0.5 text-[8px] font-bold text-pink-400 border border-pink-500/30">BẠN</span>}
        </p>
        <div className="flex flex-wrap items-center gap-1.5 mt-0.5 text-[9px] text-[#8B7A9E]">
          <span className="flex items-center gap-1"><Zap className="h-3 w-3 text-pink-400" /> {entry.xp.toLocaleString()} XP</span>
          <span className="w-1 h-1 rounded-full bg-[#8B7A9E]/40" />
      ...
      <div className="shrink-0 text-right min-w-[40px]">
        <p className={`text-sm sm:text-base font-black ${isTop3 ? getRankText(entry.rank) : 'text-[#EC4899]'}`}>
          {entry.compositeScore > 0 ? (entry.compositeScore).toFixed(1) : 0}
        </p>
        <p className="text-[9px] text-[#8B7A9E]">Điểm</p>
      </div>
    </motion.div>
  )
}
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate(-1)}
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#3D3348] text-[#8B7A9E] transition-all hover:bg-[#252030] hover:border-[#EC4899] hover:text-[#EC4899]"
          >
            <ArrowLeft className="h-4 w-4" />
          </button>
          <div className="flex items-center gap-2">
            <Trophy className="h-5 w-5 text-yellow-400" />
            <h1 className="text-base sm:text-lg font-black text-[#F5F0FA]">Hành Lang Danh Vọng</h1>
          </div>
        </div>
        <Link to="/quiz">
          <Button size="sm" className="gap-1.5 shadow-md shadow-[#EC4899]/20 hover:scale-105 active:scale-95 transition-transform text-xs font-bold px-3">
      ...
      {isLoading ? (
        ...
        <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-4 overflow-hidden rounded-xl md:rounded-2xl">
          ...
          <motion.div 
            ...
            className="flex flex-col bg-[#1A1520] border border-[#3D3348] shadow-lg overflow-hidden relative rounded-xl"
          >
            <div className="shrink-0 p-2 sm:p-3 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-pink-900/40 via-[#1A1520] to-[#1A1520] border-b border-[#3D3348]/60 flex items-center gap-2.5">
              <div className="h-8 w-8 flex-shrink-0 bg-pink-500/20 text-pink-400 border border-pink-500/30 rounded-lg flex items-center justify-center shadow-md shadow-pink-500/10">
                <CalendarDays className="h-4 w-4" />
              </div>
              <div>
                <h2 className="text-xs sm:text-sm font-black text-[#F5F0FA]">Giải Đấu Tuần</h2>
                <p className="text-[9px] sm:text-[10px] text-[#8B7A9E] mt-0.5">Reset vào Chủ Nhật. Điểm = Tổng cộng chênh lệch kỷ lục.</p>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-2 py-2 sm:p-3 space-y-1.5 custom-scrollbar">
              {weeklyEntries.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center p-4 space-y-2">
                  <Flame className="h-10 w-10 text-[#3D3348]" />
                  <div>
                    <p className="text-sm font-bold text-[#F5F0FA]">Tuần này chưa có ai tham gia!</p>
                  </div>
                </div>
              ) : (
            ...
            {meWeeklyQuery.data && (
              <div className={`shrink-0 p-2.5 sm:p-3 shadow-[0_-4px_12px_rgba(0,0,0,0.3)] z-10 flex items-center justify-between gap-3 ${
                meWeeklyQuery.data.rank > 0 && meWeeklyQuery.data.rank <= 3 ? getRowStyle(meWeeklyQuery.data.rank) + ' border-t' : 'bg-[#252030] border-t border-[#EC4899]/50'
              }`}>
                 <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className={`text-[9px] sm:text-[10px] font-semibold ${meWeeklyQuery.data.rank > 0 && meWeeklyQuery.data.rank <= 3 ? getRankText(meWeeklyQuery.data.rank) : 'text-[#8B7A9E]'}`}>
                        Xếp hạng của bạn (Tuần)
                      </p>
                      <div className="flex items-center gap-2">
                        <p className={`text-xs sm:text-sm font-bold truncate ${meWeeklyQuery.data.rank > 0 && meWeeklyQuery.data.rank <= 3 ? getRankText(meWeeklyQuery.data.rank) : 'text-[#F5F0FA]'}`}>
                          {meWeeklyQuery.data.username}
                        </p>
                        <span className="rounded bg-pink-500/20 px-1.5 py-0.5 text-[8px] font-bold text-pink-400 border border-pink-500/30">BẠN</span>
                      </div>
                      <div className="flex flex-wrap items-center gap-1.5 mt-0.5 text-[9px] text-[#8B7A9E]">
                        <span className="flex items-center gap-1"><Zap className="h-3 w-3 text-pink-400" /> {meWeeklyQuery.data.xp.toLocaleString()} XP</span>
                        <span className="w-1 h-1 rounded-full bg-[#8B7A9E]/40" />
                  ...
                  <div className="text-right shrink-0">
                    <p className={`text-base sm:text-lg font-black drop-shadow-md ${meWeeklyQuery.data.rank > 0 && meWeeklyQuery.data.rank <= 3 ? getRankText(meWeeklyQuery.data.rank) : 'text-[#EC4899]'}`}>
          ...
          <motion.div 
            ...
            className="flex flex-col bg-[#1A1520] border border-[#3D3348] shadow-lg overflow-hidden relative rounded-xl"
          >
            <div className="shrink-0 p-2 sm:p-3 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-blue-900/30 via-[#1A1520] to-[#1A1520] border-b border-[#3D3348]/60 flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-xs sm:text-sm font-black text-[#F5F0FA]">Toàn Thời Gian</h2>
                <p className="text-[9px] sm:text-[10px] text-[#8B7A9E] mt-0.5">Kỷ lục tính theo Tỉ lệ Điểm trung bình cao nhất.</p>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-2 py-2 sm:p-3 space-y-1.5 custom-scrollbar">
              {globalEntries.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center p-4">
                  <p className="text-sm font-bold text-[#F5F0FA]">Chưa có dữ liệu hệ thống.</p>
                </div>
              ) : (
                <motion.div className="flex flex-col gap-1.5" variants={{ show: { transition: { staggerChildren: 0.05 } } }} initial="hidden" animate="show">
            ...
            {meGlobalQuery.data && (
              <div className={`shrink-0 p-2.5 sm:p-3 shadow-[0_-4px_12px_rgba(0,0,0,0.3)] z-10 flex items-center justify-between gap-3 ${
                meGlobalQuery.data.rank > 0 && meGlobalQuery.data.rank <= 3 ? getRowStyle(meGlobalQuery.data.rank) + ' border-t' : 'bg-[#252030] border-t border-blue-500/50'
              }`}>
                 <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    <div className={`flex h-8 w-8 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-lg text-sm sm:text-base font-black shadow-inner ${
                    ...
                    <div className="min-w-0">
                      <p className={`text-[9px] sm:text-[10px] font-semibold ${meGlobalQuery.data.rank > 0 && meGlobalQuery.data.rank <= 3 ? getRankText(meGlobalQuery.data.rank) : 'text-[#8B7A9E]'}`}>
                        Thành tích trọn đời
                      </p>
                      <div className="flex items-center gap-2">
                        <p className={`text-xs sm:text-sm font-bold truncate ${meGlobalQuery.data.rank > 0 && meGlobalQuery.data.rank <= 3 ? getRankText(meGlobalQuery.data.rank) : 'text-[#F5F0FA]'}`}>
                  ...
                  <div className="text-right shrink-0 flex flex-col justify-center">
                    <p className={`text-base sm:text-lg font-black drop-shadow-md ${meGlobalQuery.data.rank > 0 && meGlobalQuery.data.rank <= 3 ? getRankText(meGlobalQuery.data.rank) : 'text-blue-400'}`}>
